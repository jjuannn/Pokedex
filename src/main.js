...
let offset = 0
botonPrev.onclick = function obtenerPokemonesAnteriores(){
        if(offset === 0){
            () => {}
        }else{
        offset -= 20
...
botonNext.onclick = function obtenerNuevosPokemones(){
    offset += 20
    borrarPokemonesAnteriores()
    fetch(`https://pokeapi.co/api/v2/pokemon?offset=${offset}&limit=20`)
    .then(respuesta => respuesta.json())
...
function borrarFotoPokemones(){
    const $foto = document.querySelector("#foto-pokemon")
    $foto.remove()
}
...
function mostrarCantidadPokemones(cantidadPokemones){
    document.querySelector("#cantidad-pokemones").textContent = cantidadPokemones
}
function cargarDataPokemones(nombre){
